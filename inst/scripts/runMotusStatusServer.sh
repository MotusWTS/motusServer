...
if [[ $TRACE == 0 ]]; then
    while (( 1 )); do
        nohup Rscript -e "library(motus);statusServer(port=$PORT, tracing=FALSE)"
        sleep 15
    done
else
    MYTMPDIR=`mktemp -d`
    cd $MYTMPDIR
    echo "library(motus); options(error=recover); statusServer(port=$PORT, tracing=TRUE)" > .Rprofile
    R
fi
