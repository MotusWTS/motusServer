...
list_jobs = function(env) {
    ...
    if (excludeSync)
        where = c(where, "t1.type <> 'syncReceiver'", "(t1.motusUserID <> 347 or t1.motusProjectID <> 0 or t1.type <> 'uploadFile')", "t1.motusUserID <> 30751 and t1.motusUserID <> 27319 and t1.motusUserID <> 547 and t1.motusUserID <> 2512 or json_extract(t1.data, '$.filename') not glob '*-[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]-[a-f0-9][a-f0-9][a-f0-9][a-f0-9][a-f0-9][a-f0-9][a-f0-9][a-f0-9].zip'")
        ...
        ## newer connected SG receiver files are uploaded by a process which pretends to be user 30751, which is a fake user, but some early uploads were done as users 547 and 27319.
    if (!is.null(done))
