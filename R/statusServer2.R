...
list_jobs = function(env) {
    ...
    if (excludeSync)
        where = c(where, "t1.type <> 'syncReceiver'", "(t1.motusUserID <> 347 or t1.motusProjectID <> 0 or t1.type <> 'uploadFile')", "t1.motusUserID <> 30751", "json_extract(t1.data, '$.filename') not glob '*-[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]-[a-f0-9][a-f0-9][a-f0-9][a-f0-9][a-f0-9][a-f0-9][a-f0-9][a-f0-9].zip'")
